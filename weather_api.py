...
def current_weather(lat:float, lon:float) ->dict[str: Any]:
    """
    Функция возвращает погоду по
    заданным координатам с API Яндекс Погода
    :param lat: широта float
    :param lon: долгота float
    :return: словарь по ключам 'город','дата/время','температура',
    'ощущуаемая температура','давление','влажность',
...
if __name__ == "__main__":
    dict_ = current_weather(59.93, 30.31)
    for k,v in dict_.items():
        print(k,v)  # Проверка работы для координат Санкт-Петербурга
